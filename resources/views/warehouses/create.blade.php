        ...
        <div class="card-body">
            <form method="POST" action="{{ route('warehouses.store') }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="form-control" placeholder="mis. Gudang Proyek A" required>
                </div>
                <div class="grid grid-2">
                    <div class="mb-3">
                        ...
                        <select name="type" class="form-control" id="warehouseType" required onchange="toggleProject()">
                            <option value="main" {{ old('type') == 'main' ? 'selected' : '' }}>Gudang Pusat (Main)</option>
                            <option value="project" {{ old('type') == 'project' ? 'selected' : '' }}>Gudang Proyek (Project)</option>
                        </select>
                    </div>
                    <div class="mb-3" id="projectWrapper" style="display:{{ old('type') == 'project' ? 'block' : 'none' }};">
