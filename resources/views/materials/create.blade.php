<x-app-layout>
    ...
    <div class="card" style="max-width:900px;">
        <div class="card-header" style="border-bottom:1px solid #f1f5f9;padding:16px 20px;">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <h2 class="card-title" style="margin:0;font-size:16px;font-weight:700;color:#0f172a;">Form Tambah Material</h2>
                    <div style="font-size:11.5px;color:#64748b;">Daftarkan material baru, spesifikasi, dan jadwal flow kedatangan bertahap</div>
                </div>
            </div>
        </div>
            ...
            <form method="POST" action="{{ route('materials.store') }}" onsubmit="prepareSubmit()">
                ...
                <div class="mb-3 p-3 border rounded" style="background:linear-gradient(135deg,#eff6ff 0%,#f0fdf4 100%);border-color:#bfdbfe !important;">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <div>
                            <span class="fw-700" style="font-size:13px;color:#0f172a;">Klasifikasi Material (3 Tingkat)</span>
                            <div class="text-muted" style="font-size:11px;">Pilih Kategori → Kelompok Barang → Ukuran/Spesifikasi</div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-3" style="gap:14px;">
                        ...
                        {{-- Dropdown 2: Kelompok Nama Barang (type) --}}
                        <div>
                            <div class="flex items-center justify-between" style="margin-bottom:4px;">
                                <label class="form-label mb-0" style="font-weight:600;color:#334155;">
                                    ...
                                    Kelompok Barang <span class="text-danger">*</span>
                                </label>
                            ...
                            {{-- Dropdown 2: Select yang bisa dibuka dan ditutup --}}
                            ...
                            <div id="type_hint" class="text-muted" style="font-size:10.5px;margin-top:3px;">
                                <i class="fas fa-info-circle me-1"></i> Pilih kategori dulu untuk melihat daftar kelompok barang
                            </div>
                            @error('type')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                        </div>

                        {{-- Field 3: Ukuran / Dimensi / Spesifikasi --}}
                        <div>
                            <label class="form-label" style="font-weight:600;color:#334155;">
                            ...
                            <input type="text" name="size" id="size_input" value="{{ old('size') }}"
                                ...
                                placeholder="Contoh: D13mm x 12m / 50kg / M8"
                ...
                {{-- Nama Material (Auto-generated, bisa diedit) --}}
                <div class="grid grid-2" style="gap:16px;">
                    <div>
                        <label class="form-label" style="font-weight:600;color:#334155;">Kode Material (SKU) <span class="text-danger">*</span></label>
                        <input type="text" name="sku" value="{{ old('sku') }}"
                            class="form-control font-monospace @error('sku') is-invalid @enderror"
                            placeholder="Contoh: MAT-BES-001" style="font-weight:600;" required>
                        @error('sku')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div>
                        <label class="form-label" style="font-weight:600;color:#334155;">
                            Nama Material (Lengkap) <span class="text-danger">*</span>
                        ...
                        <input type="text" name="name" id="name_input" value="{{ old('name') }}"
                            ...
                            placeholder="Otomatis dari Kelompok + Ukuran, bisa diedit" required
                            onfocus="this.dataset.manualEdit='true'">
                        @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        <div class="text-muted" style="font-size:10.5px;margin-top:3px;">
                            <i class="fas fa-lightbulb me-1 text-warning"></i> Terisi otomatis dari <em>Kelompok Barang + Ukuran</em>. Ubah manual jika perlu nama dagang khusus.
                        </div>
                    </div>
                    <div>
                        <label class="form-label" style="font-weight:600;color:#334155;">Satuan</label>
                    ...
                    <div id="new_unit_wrap" style="display:none;">
                        <label class="form-label" style="font-weight:600;color:#334155;">Nama Satuan Baru</label>
                ...
                {{-- Tahap Kedatangan Barang (Flow Masuk T1, T2, T3, ...) --}}
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <div class="text-muted" style="font-size:11px;margin-top:1px;">
                                    Catat jadwal kedatangan bertahap (sudah masuk maupun rencana mendatang)
                                </div>
                            </div>
                        </div>
                        ...
                        <table style="width:100%;font-size:12px;border-collapse:collapse;" id="stages_table">
                            <thead>
                                <tr style="background:#f1f5f9;color:#475569;font-size:11.5px;text-transform:uppercase;letter-spacing:.02em;">
                                    ...
                                    <th style="padding:8px 12px;width:130px;">Qty Masuk</th>
                                    <th style="padding:8px 12px;width:170px;">Status</th>
                                    <th style="padding:8px 12px;">Keterangan / No. SJ / Truk</th>
                    ...
                    <div class="flex items-center justify-between p-3 border-top" style="background:#f8fafc;border-color:#e2e8f0;gap:12px;flex-wrap:wrap;">
                        <div class="flex items-center gap-2" style="flex-wrap:wrap;">
                            <div style="padding:4px 10px;border-radius:6px;background:#f0fdf4;border:1px solid #bbf7d0;font-size:11.5px;color:#166534;">
                                <i class="fas fa-check-circle me-1 text-success"></i> Sudah Masuk: <strong id="summary_received">0</strong>
                            </div>
                            <div style="padding:4px 10px;border-radius:6px;background:#fffbeb;border:1px solid #fde68a;font-size:11.5px;color:#b45309;">
                                <i class="fas fa-clock me-1 text-warning"></i> Rencana: <strong id="summary_planned">0</strong>
                            </div>
                            <div style="padding:4px 10px;border-radius:6px;background:#eff6ff;border:1px solid #bfdbfe;font-size:11.5px;color:#1d4ed8;">
                                <i class="fas fa-layer-group me-1 text-primary"></i> Total Keseluruhan: <strong id="summary_total">0</strong>
                            </div>
                        </div>
                        <div style="font-size:11px;color:#64748b;">
                            *Tahap berstatus <strong class="text-success">Sudah Masuk</strong> otomatis diakumulasikan ke stok awal jika gudang dipilih.
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4 p-3 border rounded" style="background:#f8fafc;border-color:#e2e8f0 !important;">
                    ...
                    <div class="grid grid-3" style="gap:14px;">
                        ...
                        <div>
                            <label class="form-label" style="font-weight:600;color:#334155;">Jumlah Stok Awal</label>
                            <input type="number" step="1" min="0" name="initial_stock" id="initial_stock_input" value="{{ old('initial_stock', 0) }}"
                        ...
                        <div>
                            <label class="form-label" style="font-weight:600;color:#334155;">Batas Min. Stok</label>
                ...
                <div class="mt-4">
                    <label class="form-label" style="font-weight:600;color:#334155;">Keterangan / Catatan Tambahan</label>
    ...
    @push('scripts')
    <script>
        // Data kelompok barang per kategori dari server
        ...
        function onCategoryChange() {
            ...
            if (catId && existingGroups[catId] && existingGroups[catId].length > 0) {
                ...
                hint.innerHTML = '<i class="fas fa-check-circle me-1 text-success"></i> ' + groups.length + ' kelompok tersedia. Klik dropdown di atas untuk membuka daftar pilihan.';
                ...
                newOpt.textContent = '++ Ketik Kelompok Baru...';
                select2.appendChild(newOpt);
                hint.innerHTML = '<i class="fas fa-info-circle me-1 text-primary"></i> Belum ada kelompok di kategori ini. Ketik nama kelompok barang baru.';
                ...
                newOpt.textContent = '++ Ketik Kelompok Baru...';
                select2.appendChild(newOpt);
                hint.innerHTML = '<i class="fas fa-info-circle me-1"></i> Pilih kategori dulu untuk melihat daftar kelompok barang';
                hint.style.color = '';
                toggleTypeMode(false);
            }
        }
        ...
        // Dynamic Incoming Stages (T1, T2, ...) Logic
        var stageIndex = 0;
            ...
            tr.innerHTML = `
                ...
                <td style="padding:8px 12px;">
                    <input type="text" name="incoming_stages[${stageIndex}][notes]" value="${notesVal}" 
                           class="form-control form-control-sm" placeholder="Contoh: DO-102 / Batch Supplier / Truk 1" style="font-size:12px;">
                </td>
        ...
        function updateStagesSummary() {
            ...
            // Sync with initial_stock if initial_stock was 0 or untouched
            var initStock = document.getElementById('initial_stock_input');
            if (initStock && totalReceived > 0 && (initStock.value == 0 || initStock.dataset.autoSynced === 'true')) {
                initStock.value = totalReceived;
                initStock.dataset.autoSynced = 'true';
            }
        }

        // Initialize stages cleanly without duplicate execution
        (function initStages() {
            var tbody = document.getElementById('stages_tbody');
            tbody.innerHTML = '';
            stageIndex = 0;
